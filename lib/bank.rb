...
class Bank
  ...
  def deposit_to_account(amount)
    raise 'Transaktion not valid: not able to deposit negative balance' if amount.negative?
  ...
  def withdraw_from_account(amount)
    raise 'Transaktion not valid: amount to withdraw can not be negative' if amount.negative?
